...
import { getRequest, patchRequest, postRequest } from "@/utils/http-client/axiosClient";
...
function resolveImageUrl(media) {
  ...
  const right = String(media).replace(/^\/+/, "");
...
async function translateToArabic(text) {
  if (!text) return "";
  try {
    const url = new URL(MENU.TRANSLATE || undefined);
...
// Dynamic validation matching your new backend structure
...
const EditFood = () => {
  ...
  const isRehydratingRef = useRef(false); // Prevents subcategory wipeout loops
  ...
  // Sync validation criteria parameters
  useEffect(() => {
  ...
  // Compute live local 15% VAT pricing matrix calculations
  ...
  const availableAttributeOptions = useMemo(() => {
    return attributeOptions.filter(
      (option) =>
        !selectedAttributes.some((selected) => selected.attributeId === option.value)
    );
  }, [attributeOptions, selectedAttributes]);

  // File preview cleanup pipeline
  useEffect(() => {
  ...
  // Pull master drop-down options lists concurrently
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        ...
        setCatOptions(categories.map((c) => ({ value: c.id, label: c.name })));
        
        setSubCatRaw(subCatRes?.data?.data?.categories || []);
      } catch (e) {
        console.error("Master initialization configuration loading failed", e);
      } finally {
  ...
  // Compute Sub Category items matching primary option
  ...
  // Safe manual change monitoring trigger loop
  useEffect(() => {
  ...
  // Rehydrate values from current active ID parameters
  useEffect(() => {
    if (!id || !catOptions.length || !subCatRaw.length) return;
    ...
    (async () => {
      try {
        ...
        const name = enTrans.name || data.name || "";
        const description = enTrans.description || data.description || "";
        const price = data.price || "";
        const mainCategoryId = data.categoryId || data.mainCategory?.id || "";
        const subCategoryId = data.subCategoryId || data.subCategory?.id || "";
        const mediaKey = data.media || "";

        setServerMedia(mediaKey);
        setPreviewUrl(mediaKey ? resolveImageUrl(mediaKey) : "");

        const currentCategory = catOptions.find((o) => o.value === mainCategoryId) || null;
        const subOption = subCatRaw.find((s) => s.id === subCategoryId);
        const currentSubCategory = subOption ? { value: subOption.id, label: subOption.name } : null;
        ...
        reset({
          name,
          description,
          price: data.isMenuCustomized ? "" : `${price}`,
        ...
        // Map existing operational customization entities if active
        if (data.isMenuCustomized && data.attributes) {
          const loadedAttributes = data.attributes.map((attr) => ({
            attributeId: attr.attributeId,
            attributeName: attr.attributeName || "Variant Set",
        ...
        console.error("Data tracking extraction step crash exception", e);
        setError("Could not complete object fetching task.");
      } finally {
        setLoading(false);
        // Defer reference restoration slightly until context cycles execute
        setTimeout(() => { isRehydratingRef.current = false; }, 100);
      }
    })();
  }, [id, catOptions, subCatRaw, reset]);

  const resolvedImageUrl = useMemo(() => {
    if (imageFile instanceof File && previewUrl) return previewUrl;
    if (serverMedia) return resolveImageUrl(serverMedia);
    return "";
  }, [previewUrl, serverMedia, imageFile]);

  // Handle dynamic verification translation processing
  async function onSubmit(values) {
    ...
    try {
          ...
          selectedAttributes.map(async (attr) => {
            const translatedVariants = await Promise.all(
              attr.variants.map(async (v) => {
                // If variant already has an Arabic name from rehydration, preserve it
                const arVariantName = v.nameAr ? v.nameAr : await translateToArabic(v.name);
                return { ...v, nameAr: arVariantName || v.name };
              })
            );
      ...
      console.error("Translation phase failed", err);
    } finally {
      setUploading(false);
    }
  }

  // Final confirmation logic using PATCH
  async function handleFinalConfirm() {
    ...
    try {
      ...
      if (file instanceof File) {
        ...
        mediaKey = uploadRes?.fileKey || uploadRes?.key || file?.name || mediaKey;
      }
      ...
      const response = await patchRequest(`${MENU.UPDATE}/${id}`, payload);
      if (response.status === RESPONSE_CODE[201] || response.status === RESPONSE_CODE[200] || response.data?.success) {
        navigate("/menu-list");
      }
    } catch (err) {
      console.error("Update operational lifecycle transaction crash", err);
    } finally {
      setUploading(false);
    }
  }
    ...
    <MainLayout>
      <div className="max-w-6xl mx-auto px-4 py-2">
        <div className="flex items-center justify-between mb-8 border-b pb-5">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-gray-900">Edit Food Item</h1>
            <p className="text-gray-500 mt-1 text-sm">Update and review variant rule changes</p>
          </div>
        </div>

        {error && (
          <div className="p-4 rounded-md border border-red-200 bg-red-50 text-red-700 mb-6 text-sm">
            {error}
          </div>
        )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-5">
                  <h3 className="font-semibold text-lg text-gray-900 border-b pb-2">Food Information</h3>

                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <Label htmlFor="foodname" className="text-xs font-semibold text-gray-700 tracking-wide">Item Name</Label>
                      <Input
                        id="foodname"
                        {...register("name")}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <Label htmlFor="foodcategory" className="text-xs font-semibold text-gray-700 tracking-wide">Primary Category</Label>
                        <Controller
                          name="category"
                          control={control}
                          render={({ field }) => (
                      ...
                      <div className="space-y-1.5">
                        <Label htmlFor="foodsubcategory" className="text-xs font-semibold text-gray-700 tracking-wide">Sub Category</Label>
                        <Controller
                          name="subCategory"
                          control={control}
                          render={({ field }) => (
                ...
                {/* DYNAMIC ATTRIBUTION SYSTEM MATRIX */}
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-5">
                  <div className="flex items-center justify-between border-b pb-3">
                    <div>
                      <h3 className="font-semibold text-lg text-gray-900">Customizations & Variants</h3>
                      <p className="text-[11px] text-gray-400 mt-0.5">Configure system modifier variations like Sizes or Add-ons.</p>
                    </div>
                        ...
                        <div className="flex flex-wrap gap-2 p-2 bg-gray-50/60 border border-dashed border-gray-200 rounded-xl">
                          {selectedAttributes.map((attr) => (
                            <div key={attr.attributeId} className="flex items-center gap-2 pl-3 pr-1 py-1 rounded-lg border border-blue-100 bg-blue-50/50 text-blue-600">
                              ...
                              <button type="button" onClick={() => removeAttribute(attr.attributeId)} className="font-bold text-xs px-1">×</button>
                            </div>
                          ))}
                        </div>
                      )}
                        ...
                        {selectedAttributes.map((attr, attrIndex) => (
                          <div key={attr.attributeId} className="border border-gray-200 rounded-xl bg-white overflow-hidden shadow-sm">
                            <div className="flex justify-between items-center px-4 py-2.5 border-b border-gray-100 bg-gray-50/80">
                              ...
                              <Button type="button" variant="ghost" size="sm" onClick={() => removeAttribute(attr.attributeId)} className="text-red-500 text-[11px] font-semibold h-7">Remove Set</Button>
                            </div>
                              ...
                              {attr.variants.map((variant, variantIndex) => (
                                <div key={variantIndex} className="grid grid-cols-12 gap-3 items-center">
                                  <div className="col-span-5">
                                    <Input placeholder="Option Name" value={variant.name} onChange={(e) => updateVariant(attrIndex, variantIndex, "name", e.target.value)} className="text-xs h-9" />
                                  </div>
                                  <div className="col-span-3">
                                    <Input type="number" placeholder="Price" value={variant.price} onChange={(e) => updateVariant(attrIndex, variantIndex, "price", e.target.value)} className="text-xs h-9" />
                                  </div>
                                  <div className="col-span-3">
                                    <Input value={variant.finalPrice || ""} placeholder="VAT Inc." readOnly disabled className="bg-gray-100 text-xs h-9 font-semibold text-green-600 cursor-not-allowed" />
                                  </div>
                                  <div className="col-span-1 flex justify-center">
                                    <Button type="button" variant="outline" size="icon" disabled={attr.variants.length === 1} onClick={() => removeVariant(attrIndex, variantIndex)} className="h-9 w-9 text-gray-400 hover:text-red-500">×</Button>
                                  </div>
                                </div>
                              ))}
                              <Button type="button" variant="outline" size="sm" onClick={() => addVariant(attrIndex)} className="text-[11px] border-dashed h-8 mt-1">+ Add Option Field</Button>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              {/* RIGHT CONFIGURATION MATRIX PANELS */}
              <div className="space-y-6">
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
                  <h3 className="font-semibold text-gray-900 border-b pb-1 text-sm tracking-wide">Food Image</h3>
                  <div className="space-y-3">
                    <div className="w-full relative h-32">
                      <Label htmlFor="images" className={`h-full w-full flex flex-col items-center justify-center border border-dashed rounded-xl transition-colors cursor-pointer text-xs font-medium bg-gray-50/30 text-gray-500 hover:bg-gray-50/80 ${errors.image ? "border-red-300 text-red-600" : "border-gray-300"}`}>
                        <span>{imageFile instanceof File ? "Change Image Asset" : "Browse Product Image"}</span>
                      </Label>
                      <input ref={fileInputRef} type="file" id="images" accept="image/*" hidden onChange={handleFileChange} />
                    </div>

                    {resolvedImageUrl && (
                      <div className="h-28 w-full rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
                        <img src={resolvedImageUrl} alt="preview" className="h-full w-full object-cover" />
                      </div>
                    )}
                    ...
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <Input
                          id="foodprice"
                          {...register("price")}
                          className={`bg-gray-50/60 border-gray-200 text-xs h-10 ${errors.price ? "border-red-300 focus-visible:ring-red-400" : "focus-visible:ring-gray-400"}`}
                        />
                        {errors.price && <p className="text-red-500 text-[11px] font-medium">{errors.price.message}</p>}
                      </div>

                      <div className="space-y-1.5">
                        <Label htmlFor="finalPrice" className="text-xs font-semibold text-gray-700 tracking-wide">Final Customer Price <span className="text-[10px] text-gray-400 font-normal">(15% VAT Inc.)</span></Label>
                        <Input id="finalPrice" value={finalPrice} readOnly disabled className="bg-gray-100/70 border-gray-200 text-gray-500 text-xs font-bold cursor-not-allowed h-10" />
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <Controller name="isVeg" control={control} render={({ field }) => <Switch checked={field.value} onCheckedChange={field.onChange} />} />
                  </div>
                </div>

                <Button type="submit" disabled={uploading} className="w-full h-11 bg-gray-900 text-white font-medium hover:bg-gray-800 rounded-xl shadow-md text-xs tracking-wider uppercase">
      ...
      {/* VERIFICATION TRANSLATION MODAL DIALOG */}
