...
const AcceptedOrder = () => {
  ...
  const getAllOdersCallBack = () => {
    getAllOders(
      `${ORDERS.ALL_LIST}?page=1&limit=10&search=${debouncedSearch}&restaurantId=${user?.resturant?.id}&status=accepted`,
    );
  };

  const handleApplyDateFilter = () => {
    getAllOders(
      `${ORDERS.ALL_LIST}?page=1&limit=10&search=${debouncedSearch}&fromDate=${date}&toDate=${toDate}&restaurantId=${user?.resturant?.id}&status=accepted`,
    );
  };
  ...
  // Replace your existing handleStatusChange function inside AcceptedOrder.jsx
  ...
  const handleStatusChange = async (id, value) => {
    ...
    try {
      // 🍔 SCENARIO Check: If assigning a driver, call the dedicated broadcast endpoint
      if (value === "assignedToDriver") {
        const broadcastPayload = {
          orderId: id,
          restaurantName: user?.resturant?.name || "Your Restaurant",
          // Pass location coordinates from your active profile state context if available,
          // or let the backend pull them from the restaurant profile record.
          pickupLat: user?.resturant?.latitude,
          pickupLng: user?.resturant?.longitude,
          vehicleTypeId: "2", // Hardcoded fallback or chosen from an assignment modal (e.g. 2 for bikes)
        };

        console.log("broadcastPayload", broadcastPayload);

        const response = await patchRequest(
          `${ORDERS.UPDATE_STATUS}/${id}/assign-driver`,
          broadcastPayload,
        );

        if (response.status === RESPONSE_CODE[200]) {
          toast.success("Food Queue Broadcast initiated successfully!");
          navigate("/orders/driver-assigned");
        }
        setLoading(false);
        return;
      ...
      // 🚕 Regular status update fallback (Accepted, Preparing, Ready)
      const payload = { orderStatus: value };
      const response = await patchRequest(
        `${ORDERS.UPDATE_STATUS}/${id}`,
        payload,
      );
      ...
      if (response.status === RESPONSE_CODE[200]) {
        ...
        if (value === "accepted") navigate("/orders/accepted");
        if (value === "preparing") navigate("/orders/preparing");
        if (value === "ready") navigate("/orders/ready");
      }
    } catch (error) {
      toast.error(
