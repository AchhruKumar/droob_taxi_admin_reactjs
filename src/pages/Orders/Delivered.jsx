...
const Delivered = () => {
  ...
  const getAllOdersCallBack = () => {
    getAllOders(
      `${ORDERS.ALL_LIST}?page=1&limit=10&search=${debouncedSearch}&restaurantId=${user?.resturant?.id}&status=delivered`
    );
  };

  const handleApplyDateFilter = () => {
    getAllOders(
      `${ORDERS.ALL_LIST}?page=1&limit=10&search=${debouncedSearch}&fromDate=${date}&toDate=${toDate}&restaurantId=${user?.resturant?.id}&status=delivered`
    );
  };
